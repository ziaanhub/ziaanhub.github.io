...
-- Improved version with better architecture and features
...
local Settings = {
    enabled = true,
    hitZone = 2,
    anticipationOffset = 5, -- Hit slightly before perfect timing
    adaptiveMode = false, -- Learns from successful hits
    visualFeedback = true,
    audioFeedback = true,
...
local function createUI()
    ...
    distanceLabel.Size = UDim2.new(1, -10, 1, 0)
    ...
    progressContainer.Size = UDim2.new(1, 0, 0, 35)
    progressContainer.Position = UDim2.new(0, 0, 0, 85)
    ...
    statsFrame.Position = UDim2.new(0, 0, 0, 15)
...
local gui, statusLabel, distanceLabel, speedLabel, statsLabel, progressBar, mainFrame, statusIndicator = createUI()
...
local function predictNeedlePosition(needle, target, speed)
    -- Predict where needle will be in next few frames
    local currentAngle = needle.Rotation
    local targetAngle = target.Rotation
    local distance = getAngleDiff(currentAngle, targetAngle)
    
    -- Estimate speed based on previous positions
    local predictedAngle = currentAngle + (speed * 0.1)
    return normalizeAngle(predictedAngle)
end

-- IMPROVED SKILLCHECK DETECTION
local function getSkillcheck()
    -- Try multiple possible GUI structures
    local possibleGuis = {
        "SkillCheckPromptGui",
        "SkillCheckGui", 
        "SkillCheck",
    ...
    for _, guiName in pairs(possibleGuis) do
        local skillGui = PlayerGui:FindFirstChild(guiName)
        if skillGui and skillGui.Enabled then
            local check = skillGui:FindFirstChild("Check") or skillGui:FindFirstChild("Circle") or skillGui:FindFirstChild("Main")
            if check and check.Visible then
                local needle = check:FindFirstChild("Line") or check:FindFirstChild("Needle") or check:FindFirstChild("Pointer")
                local target = check:FindFirstChild("Goal") or check:FindFirstChild("Target") or check:FindFirstChild("Zone")
                
                if needle and target and needle.Visible and target.Visible then
...
-- ENHANCED HIT FUNCTION WITH MODERN FEEDBACK
local function hitSkillcheck(distance)
    ...
    task.wait(0.6)
    return true
end TweenService:Create(mainFrame, 
...
connection = RunService.Heartbeat:Connect(function()
    ...
    local approachDistance = calculateApproachDistance(needle.Rotation, target.Rotation)
    
    -- Update needle speed calculation
    local needleSpeed = updateNeedleSpeed(needle.Rotation)
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    elseif input.KeyCode == Enum.KeyCode.F7 then
        ...
        if needle and target then
            local approachDistance = calculateApproachDistance(needle.Rotation, target.Rotation)
            local speed = updateNeedleSpeed(needle.Rotation)
            print("=== ENHANCED SKILLCHECK DEBUG ===")
            print("Needle angle:", needle.Rotation)
            print("Target angle:", target.Rotation)
    ...
    elseif input.KeyCode == Enum.KeyCode.F8 then
        ...
        if needle and target then
            local approachDistance = calculateApproachDistance(needle.Rotation, target.Rotation)
            local optimalDistance = 110 + (updateNeedleSpeed(needle.Rotation) * 0.5)
